  ...
  return (
    ...
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M14.25 3.75H6.75A1.5 1.5 0 0 0 5.25 5.25v13.5a1.5 1.5 0 0 0 1.5 1.5h10.5a1.5 1.5 0 0 0 1.5-1.5V8.25l-4.5-4.5Z"
      />
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M14.25 3.75v4.5h4.5M8.25 13.5h7.5M8.25 16.5h7.5M8.25 10.5h2.25"
      />
    </svg>
  );
}
    ...
    async function loadHomepageContent() {
      ...
      if (!settingsResult.error) {
        for (const row of settingsResult.data ?? []) {
          ...
          if (
            row.setting_key === "public_cut_sheets" &&
            Array.isArray(row.setting_value)
          ) {
    ...
    <main className="min-h-screen bg-stone-50 text-stone-900">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-stone-950/95 text-white backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4 sm:px-8 lg:px-12">
          ...
          <nav className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm font-bold uppercase tracking-[0.08em]">
            ...
            <Link href="/pricing" className="transition hover:text-red-400">
              Pricing
            </Link>
            <a href="#gallery" className="transition hover:text-red-400">
              Gallery
            </a>
            <a href="#cut-sheets" className="transition hover:text-red-400">
              Cut Sheets
            </a>
            <Link
              href="/schedule"
              className="rounded-md bg-red-800 px-4 py-2 transition hover:bg-red-700"
            >
              Schedule
            </Link>
          </nav>
      ...
      <section className="relative isolate flex min-h-[700px] items-center overflow-hidden bg-stone-950">
        ...
        <div className="mx-auto flex w-full max-w-7xl flex-col items-center px-6 py-24 text-center sm:px-8 lg:px-12">
          <img
            src="/apex-logo-gray.png"
            alt={`${shopInformation.name} logo`}
            className="mb-8 h-auto w-full max-w-[300px] opacity-90"
          />
          <p className="mb-5 text-sm font-semibold uppercase tracking-[0.34em] text-stone-200">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-7 text-stone-200">
          ...
          <div className="mt-10 flex w-full max-w-xl flex-col gap-4 sm:flex-row">
            <Link
              href="/schedule"
              className="inline-flex min-h-14 flex-1 items-center justify-center gap-3 rounded-md bg-red-800 px-7 py-4 text-sm font-bold uppercase tracking-[0.12em] text-white transition hover:bg-red-700"
            >
              <CalendarIcon />
              Schedule Processing
            </Link>
            <a
              href={phoneHref}
              className="inline-flex min-h-14 flex-1 items-center justify-center gap-3 rounded-md border border-white/60 bg-white/10 px-7 py-4 text-sm font-bold uppercase tracking-[0.12em] text-white transition hover:bg-white hover:text-stone-950"
            >
              <PhoneIcon />
              Call Now
            </a>
      ...
      <section id="pricing" className="scroll-mt-24 bg-white py-20 sm:py-24">
        <div className="mx-auto max-w-5xl px-6 sm:px-8">
          ...
          <p className="mx-auto mt-8 max-w-2xl text-center text-lg leading-8 text-stone-600">
            View our current Beef, Pork, Goat &amp; Sheep, and Deer processing
            prices.
          </p>
          <div className="mt-10 text-center">
            <Link
              href="/pricing"
              className="inline-flex min-h-14 items-center justify-center rounded-md bg-red-800 px-8 py-4 text-sm font-bold uppercase tracking-[0.12em] text-white transition hover:bg-red-700"
            >
              View All Pricing
            </Link>
          </div>
      ...
      <section
        id="meat-request"
        className="scroll-mt-24 bg-red-900 py-20 text-white sm:py-24"
      >
        <div className="mx-auto max-w-6xl px-6 sm:px-8">
          <div className="text-center">
            ...
            <div className="mx-auto mt-6 h-1 w-16 bg-white" />
            <p className="mx-auto mt-8 max-w-3xl text-lg leading-8 text-red-100">
              Interested in a quarter, half, or whole beef—or a roaster pig at a
              particular hanging weight? Send the shop a request and we will
              contact you about availability.
            </p>
          </div>
          <div className="mt-10 grid gap-5 sm:grid-cols-2">
            <article className="rounded-lg border border-white/20 bg-white/10 p-6">
              ...
              <p className="mt-3 leading-7 text-red-100">
                Choose the amount of beef you want and leave your name and phone
                number.
              </p>
            </article>
            <article className="rounded-lg border border-white/20 bg-white/10 p-6">
              ...
              <p className="mt-3 leading-7 text-red-100">
                Enter the approximate hanging weight you want and the best phone
                number to reach you.
              </p>
            </article>
          </div>
          <div className="mt-10 text-center">
            <Link
              href="/meat-request"
              className="inline-flex min-h-14 items-center justify-center rounded-md bg-white px-8 py-4 text-sm font-bold uppercase tracking-[0.12em] text-red-900 transition hover:bg-stone-100"
            >
              Submit a Meat Request
            </Link>
          </div>
        </div>
      </section>
      ...
      >
        <div className="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12">
          ...
          {loading ? (
            <div className="mt-12 h-60 animate-pulse rounded-lg bg-stone-200" />
            ...
            <>
              <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {homepageGallery.map((image, index) => (
                  <figure
                    key={image.id}
                    className="group aspect-[4/3] overflow-hidden rounded-sm bg-stone-200 shadow-sm"
                  >
                    <img
                      src={image.image_url}
              ...
              <div className="mt-10 text-center">
                <Link
                  href="/gallery"
                  className="inline-flex rounded-md bg-stone-950 px-7 py-4 text-sm font-bold uppercase tracking-[0.12em] text-white transition hover:bg-red-800"
                >
                  View Full Gallery
                </Link>
              </div>
            ...
            <p className="mt-12 text-center text-stone-600">
              Gallery pictures will appear here after they are added.
            </p>
          )}
        </div>
      </section>
      ...
      <section id="cut-sheets" className="scroll-mt-24 bg-white py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-6 sm:px-8">
          ...
          {cutSheets.length ? (
            <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {cutSheets.map((sheet) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group flex items-center gap-4 rounded-lg border border-stone-200 bg-stone-50 p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-red-800 hover:shadow-md"
                >
                  <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-red-800 text-white">
                    <DocumentIcon />
                  </div>
                  <div>
            ...
            <p className="mt-12 text-center text-stone-600">
              Cut sheet PDFs will be available here soon.
            </p>
          )}
        </div>
      </section>
      ...
      <section className="bg-red-900 py-16 text-white">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-8 px-6 text-center sm:px-8 lg:flex-row lg:px-12 lg:text-left">
          ...
          <Link
            href="/schedule"
            className="inline-flex items-center justify-center gap-3 rounded-md bg-white px-7 py-4 text-sm font-bold uppercase tracking-[0.12em] text-red-900"
          >
            <CalendarIcon />
            Schedule Now
          </Link>
      ...
      <footer className="bg-stone-950 py-12 text-stone-300">
        <div className="mx-auto grid max-w-7xl gap-8 px-6 sm:px-8 md:grid-cols-2 lg:grid-cols-4 lg:px-12">
          ...
          <div>
            ...
            <a href={phoneHref} className="mt-4 block hover:text-white">
              {shopInformation.phone}
            </a>
            <p className="mt-2">{shopInformation.address}</p>
          </div>
          <div>
            <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-white">
              Connect
            </h2>
            {shopLinks.filter((link) => link.label.trim() && link.url.trim())
              .length ? (
              <div className="mt-4 flex flex-col gap-2">
                {shopLinks
                  ...
                  .map((link) => (
                    <a
                      key={link.id}
                      href={normalizePublicUrl(link.url)}
                      target="_blank"
                      rel="noreferrer"
          ...
          <div>
            ...
            <div className="mt-4 flex flex-col gap-2">
              <Link href="/meat-request">Request Beef or a Roaster Pig</Link>
              <Link href="/pricing">Pricing</Link>
              <Link href="/gallery">Full Gallery</Link>
              <a href="#cut-sheets">Cut Sheets</a>
            </div>
          </div>
        </div>
