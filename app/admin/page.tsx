...
function formatTime(record: AppointmentRecord, date: Date | null) {
  ...
  if (rawTime) {
    // Convert database-style times like 08:00:00 or 16:30:00 to 12-hour time.
  ...
  async function loadAppointments() {
    ...
    const enrichedAppointments = appointmentRows.map((appointment) => {
      ...
      return {
        ...
        animal_count: animalCountByAppointment.get(String(appointment.id)) ?? 1,
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-8 lg:px-12">
        ...
        <section className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          ...
          >
            ...
            <p className="mt-2 leading-7 text-stone-600">
              Add a phone booking with the customer&apos;s information and the
              number of beef, pork, sheep, or goats they are bringing.
            </p>
          </Link>
        ...
        >
          <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white shadow-2xl">
            ...
            <div className="space-y-5 px-6 py-6">
              ...
              {getAppointmentAnimal(selectedAppointment).trim().toLowerCase() ===
              "beef" ? (
                getOwnerContacts(selectedAppointment).length > 0 ? (
                  <div className="rounded-lg bg-red-50 p-4">
                    ...
                    <div className="mt-2 space-y-3">
                      {Array.from(
                        new Map(
                          getOwnerContacts(selectedAppointment).map((contact) => [
                            `${contact.name.toLowerCase()}|${contact.phone}`,
                            contact,
                          ]),
                        ).values(),
                      ).map((ownerContact) => (
                        <div key={`${ownerContact.customerId}-${ownerContact.phone}`}>
                ...
                <div className="rounded-lg bg-red-50 p-4">
                  ...
                  {getString(selectedAppointment, [
                    ...
                    <a
                      href={`tel:${getString(selectedAppointment, [
                        "phone",
                        "customer_phone",
                        "phone_number",
                        "contact_phone",
